...
const taxas = {
  ...
  tipo2: {
    ...
    limite: {
      ...
      17: 15.97, 18: 16.66, 19: 18.03, 20: 19.35,
      21: 19.99,
    },
  },
} as const;
...
function calcular(
  ...
  const totalPagar =
    opcao === "liberado" ? valor + valor * taxa : valor;

  const valorLiberado =
    opcao === "liberado" ? valor : valor - valor * taxa;
...
export default function Calculadora() {
  ...
  const handleCalcular = () => {
    ...
    setRes(calcular(tipo, opcao, v, parcelas));
  };
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="max-w-3xl w-full grid md:grid-cols-2 gap-6">
        <div className="bg-zinc-900 p-6 rounded-xl space-y-3">
          ...
          <select value={opcao} onChange={e => setOpcao(e.target.value as Opcao)}>
        ...
        <div className="bg-zinc-900 p-6 rounded-xl">
          {res ? (
            <>
              ...
              <p>Total: R$ {formatar(res.totalPagar)}</p>
              <button onClick={handleCopiar}>Copiar Resultado</button>
            </>
          ) : (
